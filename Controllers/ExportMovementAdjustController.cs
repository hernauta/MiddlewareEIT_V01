...
namespace MiddlewareEIT.API.Controllers
{
    ...
    [ApiController]
    public class ExportMovementAdjustController : ControllerBase
    {
        ...
        DAL dl = new DAL();
        public ExportMovementAdjustController(ILogger<AuditoriasController> logger, BdMiddlewareEITContext context)
        {
            _logger = logger;
            _context = context;
        }
        ...
        /// </summary>
        /// <param name="exportMovementAdjustXml">Objeto DTO</param>
        ...
        [HttpPost]
        [Route("api/ExportMovementAdjustEIT")]
        ...
        public string ExportMovementAdjustEIT([FromBodyAttribute] XElement exportMovementAdjustXml)
        {
            var audit = new AuditoriaDTO();
            var movementAdjuststr = exportMovementAdjustXml.ToString();
            ...
            var exportMovementAdjust = new ExportMovementAdjustDTO();
            exportMovementAdjust = Serializar.DeserializarTo<ExportMovementAdjustDTO>(movementAdjuststr, false);
            var Owner = exportMovementAdjust.OwnCode;

            audit.Id = 0;
            audit.IdCliente = 0;
            audit.Metodo = "ExportMovementAdjustEIT-Request";
            audit.TipoEvento = "O";
            audit.Dato = exportMovementAdjustXml.ToString(); //XmlCargaAudit
            ...
            using (var client = new HttpClient())
            {
                try
                {
                    ...
                    var exportMovementAdjustValid = ValidaMetodosOwner.validarMovementAdjust(exportMovementAdjust, "ExportMovementAdjustEIT", Owner);

                    if (exportMovementAdjustValid.ToString() == "No existen coincidencias con valor ingresado")
                    {
                    ...
                    if (exportMovementAdjustValid.ToString().Contains("El campo") == true)
                    {
                        return "StatusCode " + BadRequest().StatusCode + "-" + (exportMovementAdjustValid.ToString());
                        _logger.LogError("StatusCode " + BadRequest().StatusCode + "-" + (exportMovementAdjustValid.ToString()));
                    }

                    exportMovementAdjust = Serializar.DeserializarTo<ExportMovementAdjustDTO>(exportMovementAdjustValid, false);

                    var request = ExportMovementAdjustServiceEIT.GetRequestExportMovementAdjust(exportMovementAdjust);
                    ...
                    var Relaciones = new AsignaRelacion().relacionService("ExportMovementAdjustEIT", Owner);
                    if (Relaciones != 0)
                    {
